...
根据比赛/项目要求，本系统的核心目标和任务如下：

#### 1.1.1 核心目标
本项目旨在**基于 FPGA 平台，开发一款 2mm 尺寸、720P 分辨率的电子内窥镜图像处理（ISP）系统**。

#### 1.1.2 硬件平台要求
*   **主控芯片：** 易灵思 (Efinix) VF-Ti60F225 FPGA 开发板。
*   **图像传感器：** 国产 2mm 1280*960 A2020 内窥模组。
*   **显示设备：** HDMI 外置显示器（自备）或 7寸 1024*600 LVDS 屏幕。

#### 1.1.3 核心指标与任务
系统需要完成以下层次的技术指标：

1.  **基础要求：视频实时传输与显示**
    *   成功驱动 A2020 传感器获取图像，并实现 **720P 分辨率、60帧/秒 (720P60)** 的实时图像输出与显示。
2.  **核心要求：内窥镜 ISP 图像处理**
    *   这是本项目的重点，需要在 FPGA 上实现图像信号处理（ISP）管线，改善原始图像质量。必须实现的算法包括：
        *   **AE (自动曝光):** 根据环境亮度自动调节曝光时间。
        *   **AWB (自动白平衡):** 还原物体真实的颜色，消除光源色温带来的偏色。
        *   **消光处理:** 抑制内窥镜照明可能带来的反光或高光溢出。
        *   **图像降噪:** 去除图像中的噪点，提升画面纯净度。
        *   **低延时控制:** 医疗内窥镜对实时性要求极高，系统整体处理延时必须达标。
3.  **进阶要求：算法优化与创新**
    *   在完成上述基础 ISP 功能后，鼓励针对“内窥镜图像”的特殊应用场景（如暗光、血管凸显、边缘增强等），实现**更多有价值的算法优化和创新**，以进一步提升图像的诊断价值。
