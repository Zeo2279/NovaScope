`timescale 1ns/1ns
module	i2c_timing_ctrl_16bit_sp
#(
...
(
    ...
    output                        i2c_transfer_complete      ,//i2c transfer actually complete
...
localparam  DELAY_TOP = CLK_FREQ/500;  //1ms Setting time after software/hardware reset
...
// 改进config_done信号生成逻辑，确保更准确地反映传输完成状态
wire i2c_transfer_complete_internal = (i2c_config_index == i2c_config_size) && (current_state == I2C_IDLE || current_state == I2C_WR_STOP);
assign i2c_config_done = i2c_transfer_complete_internal;
assign i2c_transfer_complete = i2c_transfer_complete_internal;
...
begin
    if(!rst_n)
        ...
        I2C_WR_ACK1:    
            ...
            i2c_sdat_out <= 1'b1;  // 明确设置为高电平，释放总线
            end            
        I2C_WR_REGADDR1: 
            begin
            i2c_stream_cnt <= i2c_stream_cnt + 1'b1;
        ...
        // 添加default分支，确保异常状态下输出信号有确定值
        default:
            i2c_sdat_out <= 1'b1;  // 安全状态，总线释放
