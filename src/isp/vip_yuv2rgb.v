...
(
	...
	**********************************************/
	...
	G = 1.164*Y - 0.391*(Cb-128) - 0.813*(Cr-128)
	B = 1.164*Y + 2.018*(Cb-128)

	R << 9 = 596Y + 817Cr - (1.596*128*512) = 596Y + 817Cr - 104484
	G << 9 = 596Y - 200Cb - 416Cr + (0.391*128*512 + 0.813*128*512) = 596Y - 200Cb - 416Cr + 79626
	B << 9 = 596Y + 1033Cb - (2.018*128*512) = 596Y + 1033Cb - 132544
	**********************************************/
	...
	**********************************************/
	...
	always @ (posedge pclk or negedge rst_n) begin
		if(!rst_n) begin
			...
			XOUT <= (img_Y_r1 + img_Cr_r1 - (19'sd104484 << (BITS-8))) >>> 9; 	
			YOUT <= (img_Y_r1 - img_Cb_r1 - img_Cr_r2 + (19'sd79626 << (BITS-8))) >>> 9; 
			ZOUT <= (img_Y_r1 + img_Cb_r2 - (19'sd132544 << (BITS-8))) >>> 9;
	...
	always @ (posedge pclk or negedge rst_n) begin
		if(!rst_n) begin
			...
			R <= XOUT < 0 ? 0 : (XOUT > 255) ? 255 : XOUT[BITS-1:0];
			G <= YOUT < 0 ? 0 : (YOUT > 255) ? 255 : YOUT[BITS-1:0];
			B <= ZOUT < 0 ? 0 : (ZOUT > 255) ? 255 : ZOUT[BITS-1:0];
		end
	end
