...
begin
    if(!rst_n)
        ...
        if(pixel_cnt == 0)
            ...
            begin   
            {matrix_p11, matrix_p12, matrix_p13} <= {row1_data0, row1_data0, row1_data};         
            {matrix_p21, matrix_p22, matrix_p23} <= {row2_data0, row2_data0, row2_data}; 
            {matrix_p31, matrix_p32, matrix_p33} <= {row3_data0, row3_data0, row3_data}; 
            end
        else if(pixel_cnt >= (IMG_HDISP-1))  // Last two pixels of line
            begin
            // Use mirroring for boundary pixels (better than duplication)
            {matrix_p11, matrix_p12, matrix_p13} <= {row1_data1, row1_data, row1_data};
            {matrix_p21, matrix_p22, matrix_p23} <= {row2_data1, row2_data, row2_data};
            {matrix_p31, matrix_p32, matrix_p33} <= {row3_data1, row3_data, row3_data};
            end
